    ...
    try {
        ...
        if (validPayload) {
            if (jsonData.hecap_id == 'SurveyOnly' && jsonData.surveyAnswers) {
                let hecapSurveyResult = await daoSql.insertSDOHSurvey(context, jsonData.hecap_id, jsonData)
                responseCode = 200
            } else {
                ...
                jsonData['hecap_id'] = hecapId
                let hecapPatientRegResult = await daoSql.insertHecapPatient(context, jsonData);
                let hecapSurveyResult = await daoSql.insertSDOHSurvey(context, hecapId, jsonData)
                results = await daoSql.insertPatientStatus(context, jsonData.hecap_id, 
                    jsonData.hecap_id, "Onboarded", "HECAP Registration", jsonData.group_name, jsonData.sms_email, 
                    jsonData.email, '1-' + jsonData.phone);
