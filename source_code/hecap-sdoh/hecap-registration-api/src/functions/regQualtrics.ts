...
export async function regQualtrics(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    ...
    try {
        ...
        context.log("====validatePayload:", validatePayload)
        if (validPayload) {
...
app.http('regQualtrics', {
    methods: ['GET', 'POST'],
    authLevel: 'anonymous',
    handler: regQualtrics
});
...
function validatePayload(payload): boolean {
    if (payload.hecap_id.length > 50
        ...
        || payload.phone.length > 15
        || payload.birthdate > 15
        || payload.prefEmail > 10
        || payload.prefSms > 10
        ) {
      return false;
    }
